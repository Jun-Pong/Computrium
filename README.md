...
![UIT Lensing Proof](assets/The%20Correction%20of%20Geometric%20Causality.png)
...
事件（E）：有可能发生的或已经发生的一件或多件事，每一个事件都是唯一的，
这一刻的事件和下一刻的事件，视为不同的事件。一个事件被视为由两个正交的二维平面（X, Y）构成。

构造：将事件E1的某个正交平面例如X1通过某种方法变为X2，构造是另一个平面上的事件。

构造方向：垂直于事件E1被构造平面的方向。

构造步骤T：对一个构造方法的步骤计数。

概率：一个事件越难被构造出来，则该事件概率越低。

几何概率区间：在一个完备的系统内，由所有概率不为0的事件构成的集合，其概率恒为1，事件在该区间的占比由其概率决定，概率越大占比越高，一个几何概率区间只能包含相同正交平面上的事件。

构造集S：将一个几何概率区间通过某种方法与构造集进行运算，得到新的几何概率区间，构造集也是一个几何概率区间。

事件链：事件从E1开始，经过T个构造步骤变为E2，将这个过程在每一个步骤T时刻产生的事件由时间顺序单向串联起来，构成一个事件链，单个事件也可以视为事件链

构造概率（CP）：在全局系统中，对于一个事件链E(0, N)，构造概率CP=P(En)/ P(E0)

逻辑质量m：定义为事件的质量，公式m=-lnP，概率越低质量越大，后续会对公式进行推导

逻辑做功w：定义事件链的质量，公式w=-lnCP，后续会对公式进行推导
