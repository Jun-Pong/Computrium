...
## 7. 几何偏转的质量无关性定理
## 7. The Mass-Independence Theorem of Geometric Deflection

> **核心公理 (Core Axiom)**
> **交互即偏转 (Interaction is Deflection)**。
> 光子运动方向的改变，是**光子动量矢量 ($p$)** 与 **空间概率势差 ($\nabla m_{space}$)** 直接发生**乘积交互**所生成的**正交增量**的结果。由于原始动量同时构成了对抗偏转的惯性基底，光子自身的质量属性在比率计算中被必然**消去 (Cancelled Out)**。
> The change in the photon's direction of motion is the result of the **Orthogonal Increment** generated by the **direct product interaction** between the **Photon Momentum Vector ($p$)** and the **Spatial Probability Potential Difference ($\nabla m_{space}$)**. Since the original momentum simultaneously constitutes the inertial basis resisting deflection, the photon's own mass attribute is inevitably **cancelled out** in the ratio calculation.

### 7.1 交互实体的矢量定义 (Vector Definition of Interacting Entities)

在 SFCP 框架下，我们将交互双方定义为纯粹的概率实体矢量：
In the SFCP framework, we define the interacting parties as pure probabilistic entity vectors:

* **光子矢量 (The Photon Vector, $\vec{V}_{ph}$)**:
    由光子的内部构造概率 ($P_{ph}$) 决定。其模长等于动量。
    Determined by the photon's internal construction probability ($P_{ph}$). Its magnitude equals momentum.
    $$|\vec{V}_{ph}| = p = [-\ln(P_{ph})] \cdot C$$

* **空间势矢量 (The Spatial Potential Vector, $\vec{V}_{space}$)**:
    由环境的概率密度梯度 ($\nabla P_{space}$) 决定。它代表了空间构造难度的变化方向（指向引力源）。
    Determined by the gradient of environmental probability density ($\nabla P_{space}$). It represents the direction of change in spatial construction difficulty (pointing to the gravitational source).
    $$|\vec{V}_{space}| = \nabla m_{space} = \nabla [-\ln(P_{space})]$$

### 7.2 交互增量的生成 (Generation of Interaction Increment)

当光子矢量穿过非均匀空间时，两者必然发生交互。这种交互表现为**概率叠加运算**生成的垂直分量。
When the photon vector passes through non-uniform space, an interaction is inevitable. This interaction manifests as a vertical component generated by **Probability Superposition Operation**.

根据 **交互积定理**，交互产生了一个垂直于原运动方向的**修正增量 ($\Delta \vec{v}_{\perp}$)**。其大小严格正比于两个实体的乘积：
According to the **Interaction Product Theorem**, the interaction generates a **Correction Increment ($\Delta \vec{v}_{\perp}$)** perpendicular to the original direction of motion. Its magnitude is strictly proportional to the product of the two entities:

$$
|\Delta \vec{v}_{\perp}| \propto |\vec{V}_{ph}| \cdot |\vec{V}_{space}|
$$

$$
\Delta \vec{v}_{\perp} = k \cdot (p \cdot \nabla m_{space})
$$

* **物理意义 (Physical Meaning)**:
    光子携带的“存在量” ($p$) 越大，且空间的“势差” ($\nabla m_{space}$) 越陡峭，两者结合产生的**交互增量**就越大。
    The greater the "Quantity of Existence" ($p$) carried by the photon, and the steeper the "Potential Difference" ($\nabla m_{space}$) of space, the larger the **Interaction Increment** generated by their combination.

### 7.3 几何合成与偏转角 (Geometric Synthesis and Deflection Angle)

光子的新运动矢量 ($\vec{V}_{new}$) 是原始矢量与交互增量的几何合成：
The photon's new motion vector ($\vec{V}_{new}$) is the geometric synthesis of the original vector and the interaction increment:

$$
\vec{V}_{new} = \vec{V}_{old} + \Delta \vec{v}_{\perp}
$$

**偏转角 ($\theta$)** 定义为增量与原矢量的比率（几何正切）：
The **Deflection Angle ($\theta$)** is defined as the ratio of the increment to the original vector (geometric tangent):

$$
\tan(\theta) \approx \theta = \frac{|\Delta \vec{v}_{\perp}|}{|\vec{V}_{old}|}
$$

### 7.4 质量无关性的数学证明 (Mathematical Proof of Mass Independence)

现在，我们将 7.1 和 7.2 中的定义代入比率公式：
Now, we substitute the definitions from 7.1 and 7.2 into the ratio formula:

$$
\theta = \frac{k \cdot (p \cdot \nabla m_{space})}{p}
$$

* **分子 (Numerator)**: 交互增量包含动量项 $p$（作为动力源）。
    (The interaction increment contains the momentum term $p$ as the driving source.)
* **分母 (Denominator)**: 原始矢量本身就是动量项 $p$（作为几何基底）。
    (The original vector itself is the momentum term $p$ as the geometric basis.)
...
1.  **纯粹的空间属性 (Pure Spatial Attribute)**:
    光子的偏转角 $\theta$ 仅取决于**空间的概率势差 ($\nabla m_{space}$)**。与光子自身的能量、频率或质量完全无关。
    The photon's deflection angle $\theta$ depends solely on the **Spatial Probability Potential Difference ($\nabla m_{space}$)**. It is completely independent of the photon's own energy, frequency, or mass.

2.  **自我归一化 (Self-Normalization)**:
    虽然光子质量（频率）参与了交互，但它同时构成了矢量的基础模长。在计算方向改变率时，光子自身的属性被**自我归一化（消除）**了。
    Although photon mass (frequency) participates in the interaction, it simultaneously constitutes the base magnitude of the vector. When calculating the rate of directional change, the photon's own attributes are **Self-Normalized (Eliminated)**.

这证明了该模型与广义相对论的“等效原理”完全兼容：引力透镜是空间的几何性质，而非依赖于波长的色散现象。
This proves the model is fully compatible with the "Equivalence Principle" of General Relativity: Gravitational lensing is a geometric property of space, not a wavelength-dependent dispersion phenomenon.
